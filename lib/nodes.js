(function() {
  var Access, Arr, Assign, Base, Block, Call, Class, Closure, Code, Comment, Existence, Extends, For, IDENTIFIER, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, LoopIndex, NEGATE, NO, Obj, Op, Param, Parens, Push, Range, Return, SIMPLENUM, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, compact, contains, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref;
  ...
  _ref = require('./helpers'), compact = _ref.compact, contains = _ref.contains, flatten = _ref.flatten, extend = _ref.extend, merge = _ref.merge, del = _ref.del, starts = _ref.starts, ends = _ref.ends, last = _ref.last;
  ...
  exports.Base = Base = (function() {
    ...
    Base.prototype.eachChild = function(func, returnOnNonfalse, returnForChain) {
      var attr, child, result, _i, _j, _len, _len2, _ref, _ref2;
      if (returnOnNonfalse == null) {
        returnOnNonfalse = false;
      }
      if (returnForChain == null) {
        returnForChain = false;
      }
      if (this.children) {
        _ref = this.children;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (this[attr]) {
            ...
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              child = _ref2[_j];
              result = func(child);
              if (!returnOnNonfalse && result === false) {
                if (returnForChain) {
                  return false;
                } else {
                  return this;
    ...
    Base.prototype.childCount = function() {
      var n;
      n = 0;
    ...
    Base.prototype.replaceChild = function(oldChild, newChild) {
      var attr, child, i, _i, _len, _len2, _ref, _ref2;
      _ref = this.children;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        attr = _ref[_i];
        if (this[attr]) {
          if (this[attr] instanceof Array) {
            ...
            for (i = 0, _len2 = _ref2.length; i < _len2; i++) {
              child = _ref2[i];
              if (child === oldChild) {
                this[attr][i] = newChild;
                return true;
              }
            }
          } else if (this[attr] === oldChild) {
            this[attr] = newChild;
            return true;
          }
        }
    ...
    Base.prototype.traverseChildrenDeep = function(crossScope, func, path) {
      if (path == null) {
        path = [this];
      }
      return this.eachChild(function(child) {
    ...
    Base.prototype.collectImplicitLoopIdentifiers = function(depth) {
      this.implicitLoopIdentifiers = [];
      this.eachChild(__bind(function(child) {
        var implicitLoopIdentifier, _i, _len, _ref, _results;
        _ref = child.implicitLoopIdentifiers;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          implicitLoopIdentifier = _ref[_i];
          _results.push(this.implicitLoopIdentifiers.push(merge(implicitLoopIdentifier, {
            source: child
          })));
        }
        return _results;
    ...
    Base.prototype.hasLoopIdentifierCollision = function(identifier) {
      var count, element, _i, _len, _ref;
      count = 0;
      _ref = this.implicitLoopIdentifiers;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        element = _ref[_i];
        if (element.identifier === identifier && element.isSource) {
          count++;
        }
      }
      return count > 1;
    };
    Base.prototype.loopIdentifierSources = function(identifier) {
      var count, element, sources, _i, _j, _len, _len2, _ref, _ref2;
      count = 0;
      if (!(this.implicitLoopIdentifiers != null)) {
        console.log("how can it be that this " + this.constructor.name + " doesn't have implicitLoopIdentifiers?");
        return 0;
      }
      sources = [];
      _ref = this.implicitLoopIdentifiers;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if ((typeof element.identifier) !== "string") {
          console.log("WAAH! This isn't an identifier!");
        }
      }
      _ref2 = this.implicitLoopIdentifiers;
      for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
        element = _ref2[_j];
        if (element.identifier === identifier && !contains(sources, element)) {
          count++;
  ...
  exports.Block = Block = (function() {
    ...
    Block.prototype.compileRoot = function(o) {
      ...
      o.level = LEVEL_TOP;
      while (true) {
        if (this.traverseChildrenDeep(true, function(child, path) {
          return child.collectImplicitLoopIdentifiers(path.length);
        })) {
          throw "this shouldn't break";
        }
        if (!(this.traverseChildrenDeep(true, function(child, path) {
          return typeof child.findOwnIdentifierScope == "function" ? child.findOwnIdentifierScope(path) : void 0;
        }))) {
          break;
        }
      }
  ...
  exports.Value = Value = (function() {
    ...
    Value.prototype.cloneUntil = function(property) {
      var i, newprops;
      if (!this.props || contains(this.props, property)) {
        console.log('WHUT?');
      }
      if (!this.props) {
        return this;
      ...
      newprops = [];
      i = 0;
      while (!(this.props[i] === property || i === this.props.length)) {
        newprops.push(props[i]);
        i++;
      }
      return new Value(base, newprops, tag);
    };
  ...
  exports.LoopIndex = LoopIndex = (function() {
    __extends(LoopIndex, Base);
    function LoopIndex(identifier, type) {
      this.identifier = identifier;
      this.child = new Index(this.identifier);
      if (type === '=') {
        this.isSource = true;
      }
      this.implicitLoopIdentifiers = [
    ...
    LoopIndex.prototype.children = ['child'];
    LoopIndex.prototype.collectImplicitLoopIdentifiers = function() {
      return false;
    };
    LoopIndex.prototype.compileNode = function(o) {
      return this.child.compile(o);
    };
    LoopIndex.prototype.findOwnIdentifierScope = function(path) {
      var i, sourceCount;
      if (!this.isSource || this.scopeFound) {
        return false;
      }
      this.scopeFound = true;
      i = path.length - 1;
      while (i > 2) {
        if (path[i].hasLoopIdentifierCollision(this.identifier.value)) {
          i++;
          break;
        }
        i--;
      ...
      while (true) {
        sourceCount = path[i].loopIdentifierSources(this.identifier.value);
        if (sourceCount >= 2) {
          break;
        } else if (sourceCount < 1) {
          console.log("this is a very serious issue. trying to workaround...");
        }
        i++;
        if (i >= path.length) {
          console.log("What the heck? This is just ridiculous. It never branches?");
          break;
        }
      }
      if (!(path[i - 1].replaceChild(path[i], new For(path[i], {
        source: path[path.length - 1].base,
        indexes: [this.identifier]
      })))) {
        console.log('error');
