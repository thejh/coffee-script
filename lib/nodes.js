(function() {
  var Access, AliasAssign, Arr, Assign, Base, Block, Call, Class, Closure, Code, Comment, Existence, Extends, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, Op, Param, Parens, Push, Range, Return, SIMPLENUM, Scope, Slice, Splat, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, compact, deepCopy, deepReplace, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref;
  ...
  _ref = require('./helpers'), compact = _ref.compact, deepCopy = _ref.deepCopy, deepReplace = _ref.deepReplace, flatten = _ref.flatten, extend = _ref.extend, merge = _ref.merge, del = _ref.del, starts = _ref.starts, ends = _ref.ends, last = _ref.last;
  ...
  exports.Base = Base = (function() {
    function Base() {}
    Base.prototype.compile = function(o, lvl, oFunc) {
      var node, result;
      o = extend({}, o);
      if (lvl) {
        o.level = lvl;
      ...
      node.tab = o.indent;
      result = o.level === LEVEL_TOP || !node.isStatement(o) ? node.compileNode(o) : node.compileClosure(o);
  ...
  exports.Block = Block = (function() {
    ...
    Block.prototype.compileNode = function(o) {
      var code, codes, copyBack, node, top, _i, _len, _ref2;
      this.tab = o.indent;
      top = o.level === LEVEL_TOP;
      codes = [];
      copyBack = function(oChild) {
        var name, value, _results;
        _results = [];
        for (name in oChild) {
          value = oChild[name];
          if (0 === name.indexOf('declare_alias_')) {
            name = name.substring(8);
            _results.push(o[name] != null ? console.error("duplicate alias declaration for '" + name + "'") : o[name] = value);
          }
        }
        return _results;
      };
  ...
  exports.Literal = Literal = (function() {
    ...
    Literal.prototype.compileNode = function(o) {
      var aliasValue, code;
      code = this.isUndefined ? o.level >= LEVEL_ACCESS ? '(void 0)' : 'void 0' : this.value.reserved ? "\"" + this.value + "\"" : this.identifier && (o["alias_" + this.value] != null) ? (this.isAlias = true, aliasValue = o["alias_" + this.value]([]), aliasValue.compile(o)) : this.value;
      if (this.isStatement()) {
  ...
  exports.AliasAssign = AliasAssign = (function() {
    var compileHelper;
    __extends(AliasAssign, Base);
    function AliasAssign(identifier, value) {
      this.value = value;
      this.name = identifier.value;
      if (this.value instanceof Code) {
        this.substitutions = this.value.params;
        this.value = this.value.body;
      }
    }
    compileHelper = function(value, substitutions, args) {
      var argName, i, replacement, _len;
      value = deepCopy(value);
      if (substitutions != null) {
        for (i = 0, _len = substitutions.length; i < _len; i++) {
          argName = substitutions[i];
          if (args[i] != null) {
            replacement = args[i];
          } else {
            replacement = new Literal('null');
          }
          deepReplace(value, function(literal) {
            if (literal instanceof Literal && literal.value === argName) {
              return replacement;
            }
          });
        }
      }
      return new Parens(value);
    };
    AliasAssign.prototype.compileNode = function(o) {
      o["declare_alias_" + this.name] = __bind(function(args) {
        return compileHelper(this.value, this.substitutions, args);
      }, this);
      return "/* alias assignment for '" + this.name + "' */";
    };
    return AliasAssign;
  })();
  ...
  exports.For = For = (function() {
    ...
    For.prototype.compileNode = function(o) {
      ...
      if (name && this.name.asAlias) {
        o["alias_" + name] = function() {
          return {
            compile: function() {
              return new Value(new Literal(svar)).push(new Index(new Literal(ivar))).compileNode({});
            }
          };
        };
      }
